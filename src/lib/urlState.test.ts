...
import { initialGarage, pinSlot, setSlotFilters, setSlotMaxMiles, DEFAULT_MAX_MILES, MIN_SLOT } from '../state/garage';
...
const sameGarage = (a: ReturnType<typeof initialGarage>, b: ReturnType<typeof initialGarage>) => {
  ...
  a.slots.forEach((slot, i) => {
    ...
    expect(got.maxMiles, `slot ${i} miles`).toBe(slot.maxMiles);
...
describe('round trip', () => {
  ...
  it('survives pins, filters and mileage limits together', () => {
    let g = initialGarage(60_000, 4);
    g = pinSlot(g, g.slots[0]!.id, 'mazda-mx5-nc', 14_000);
    g = setSlotMaxMiles(g, g.slots[1]!.id, 200_000);
...
describe('link length', () => {
  ...
  it('does not pay for values that are already the default', () => {
    ...
    heavy = setSlotMaxMiles(heavy, heavy.slots[0]!.id, 250_000);
...
describe('a link must open forever', () => {
  ...
  it('fills in fields an older version omitted', () => {
    ...
    expect(back.slots[0]!.maxMiles).toBe(DEFAULT_MAX_MILES);
  ...
  it('clamps hostile numbers instead of trusting them', () => {
    ...
    expect(back.slots[0]!.maxMiles).toBeLessThanOrEqual(400_000);
  });
